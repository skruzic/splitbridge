                ...
                @foreach ($member['ranks'] as $rank)
                    <a href="{{ url($rank->tournament->results) }}">
                        <x-table.row>
                            ...
                            <x-table.cell>{{ $rank->tournament->type->name }}</x-table.cell>
                            <x-table.cell>{{ Number::ordinal($rank->rank) }}</x-table.cell>
                            <x-table.cell
                                    class="text-right">{{ Number::format($rank->points, precision: 2) }}</x-table.cell>
                            @if($rank->tournament->sessions->count() > 0)
                                <x-table.cell>
                                    <x-button variant="primaryOutline" size="sm"
                                              href="{{ route('tournaments.show', $rank->tournament->id) }}">Rezultati
                                    </x-button>
                                </x-table.cell>
                            @elseif($rank->tournament->remote_id > 0)
                                <x-table.cell>
                                    <x-button variant="primaryOutline" size="sm"
